...
class UsermgmtService {
    ...
    async addEmployeeStep1(step1: Step1Dto): Promise<EmployeeListDto> {
        try {
    ...
    // POST: /api/hrm/profile/v1/AdminEmp/Step2/{id}
    async addEmployeeStep2(employeeId: UUID, step2: Step5Dto): Promise<EmployeeListDto> {
        try {
            const response = await api.post(`${this.baseUrl}/Step2/${employeeId}`, step2);
            console.info('Employee step 2 completed successfully:', employeeId);
            return response.data.data;
        } catch (error) {
            const errorMessage = this.extractErrorMessage(error);
